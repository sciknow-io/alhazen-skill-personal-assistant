#!/usr/bin/env python3
"""
validate_views.py -- VPDMf view spec validator for the jobhunt dashboard.
...
# necessarily repeated in the jobhunt namespace schema.tql)
# ---------------------------------------------------------------------------
BASE_ENTITY_TYPES = {
    "identifiable-entity",
    "domain-thing",
...
def coverage_report(all_views, entity_types):
    ...
    # Jobhunt-specific entity types (filter to just jobhunt-* + a few core ones)
    jobhunt_types = {t for t in entity_types if t.startswith("jobhunt-") or t == "collection"}
    unreachable = jobhunt_types - covered - inline_only
...
def main():
    ...
    for slug, data in sorted(all_views.items()):
        ...
        if slug == "jobhunt" or "hub" in data:
