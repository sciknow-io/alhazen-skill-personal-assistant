...
import { EmbeddingMap, MapItem } from '@/components/career/embedding-map';
import { OpportunityList } from '@/components/career/opportunity-list';
...
export default function MissionControl() {
  ...
  const [activeTab, setActiveTab] = useState<'opportunities' | 'people' | 'projects' | 'search' | 'about' | 'learning'>('opportunities');

  const fetchItems = useCallback(async (exclude?: Set<string>) => {
    setLoading(true);
    try {
      let url = '/api/career/embedding-map';
...
function SearchTab() {
  ...
  useEffect(() => {
    fetch('/api/career/sources')
      .then(r => r.json())
      .then(data => setSources(data.sources ?? []))
      .catch(() => {})
      .finally(() => setLoadingSources(false));

    fetch('/api/career/candidates')
      .then(r => r.json())
    ...
    <div style={{ flex: 1, overflow: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div>
        ...
        {loadingSources ? (
          ...
          <div style={{ color: '#5e7387', fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', padding: '16px' }}>
            No sources configured. Use <code style={{ color: '#5aadaf' }}>career_forager.py add-source</code> to add company boards or aggregators.
          </div>
        ) : (
      ...
      <div>
        ...
        {loadingCandidates ? (
          ...
          <div style={{ color: '#5e7387', fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', padding: '16px' }}>
            No candidates discovered yet. Run <code style={{ color: '#5aadaf' }}>career_forager.py heartbeat</code> to search all sources.
          </div>
        ) : (
          <div style={{
...
function AboutTab() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/career/seekers').then(r => r.json()),
      fetch('/api/career/skills').then(r => r.json()),
...
function LearningTab() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/career/fit').then(r => r.json()),
...
// =============================================================================
// People Tab — the career graph: contacts, mentors, sponsors, collaborators
// =============================================================================
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', padding: '20px 28px' }}>
      ...
      {loading ? (
        ...
        <div style={{ color: '#5e7387', fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', padding: '16px' }}>
          No people yet. Use <code style={{ color: '#5aadaf' }}>career.py add-person</code> to add contacts and collaborators.
        </div>
      ) : (
        <div style={{
          border: '1px solid rgba(200, 221, 232, 0.08)',
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: '1.4fr 2fr 120px 120px',
            padding: '6px 12px',
            background: 'rgba(12, 22, 40, 0.72)',
            fontFamily: "'JetBrains Mono', monospace",
            fontSize: '10px',
          ...
          {people.map(p => (
            <div key={p.id} style={{
              ...
              <Link href={`/career/person/${p.id}`} style={{ color: '#5aadaf', textDecoration: 'none' }}>
                {p.name}
              </Link>
              <span style={{
                color: '#8ba4b8',
              ...
              <span style={{ color: p.collaborations > 0 ? '#b8c84a' : '#5e7387', fontFamily: "'JetBrains Mono', monospace" }}>
                {p.collaborations || '—'}
              </span>
              <span style={{ color: p.contact_roles > 0 ? '#5b8ab8' : '#5e7387', fontFamily: "'JetBrains Mono', monospace" }}>
                {p.contact_roles || '—'}
              </span>
            </div>
          ))}
...
// =============================================================================
// Projects Tab — bodies of work that build career capital, with collaborators
// =============================================================================
...
const PROJECT_COLUMNS = ['exploring', 'active', 'paused', 'shipped', 'sunset'];
...
function ProjectsTab() {
  ...
  if (loading) {
    return (
      <div style={{ flex: 1, padding: '20px 28px', color: '#5e7387', fontFamily: "'JetBrains Mono', monospace", fontSize: '11px' }}>
        Loading...
      </div>
    );
  }
  ...
  if (projects.length === 0) {
    return (
      <div style={{ flex: 1, padding: '20px 28px', color: '#5e7387', fontFamily: "'JetBrains Mono', monospace", fontSize: '12px' }}>
        No projects yet. Use <code style={{ color: '#5aadaf' }}>career.py add-project</code> to track a body of work that builds career capital.
      </div>
    );
  }
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', padding: '20px 28px' }}>
      <div style={{
        display: 'grid',
        gridTemplateColumns: `repeat(${PROJECT_COLUMNS.length}, minmax(200px, 1fr))`,
        gap: '12px',
        alignItems: 'start',
      }}>
        {PROJECT_COLUMNS.map(col => {
          const colProjects = projects.filter(p => (p.status || 'exploring') === col);
          return (
            <div key={col}>
              <div style={{
                fontFamily: "'JetBrains Mono', monospace",
                fontSize: '10px',
                color: PROJECT_STATUS_COLORS[col] || '#5e7387',
                textTransform: 'uppercase',
                letterSpacing: '1.4px',
                marginBottom: '8px',
              }}>
              ...
              {colProjects.map(p => (
                <div key={p.id} style={{
                  ...
                  <Link href={`/career/opportunity/${p.id}`} style={{
                    color: '#c8dde8',
                    textDecoration: 'none',
                    fontSize: '13px',
                    fontWeight: 600,
                  ...
                  {p.role && (
                    <div style={{ fontSize: '11px', color: '#8ba4b8', marginBottom: '4px' }}>
                      role: <span style={{ color: '#5aadaf' }}>{p.role}</span>
                    </div>
                  )}
                  {p.collaborators && p.collaborators.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '6px' }}>
                      {p.collaborators.map(c => (
                        <Link key={c.id} href={`/career/person/${c.id}`} style={{
                          fontSize: '10px',
                          fontFamily: "'JetBrains Mono', monospace",
                          color: '#b8c84a',
