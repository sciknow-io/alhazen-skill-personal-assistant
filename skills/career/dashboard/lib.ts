...
// CAREER_SKILL_ROOT: absolute path to the career skill directory (used in standalone demo)
...
const SKILL_ROOT = process.env.CAREER_SKILL_ROOT;
const PROJECT_ROOT = process.env.PROJECT_ROOT || path.resolve(process.cwd());

const CAREER_SCRIPT = SKILL_ROOT
  ? path.join(SKILL_ROOT, 'career.py')
  : path.join(PROJECT_ROOT, '.claude/skills/career/career.py');

const FORAGER_SCRIPT = SKILL_ROOT
  ? path.join(SKILL_ROOT, 'career_forager.py')
  : path.join(PROJECT_ROOT, '.claude/skills/career/career_forager.py');

const EMBEDDING_SCRIPT = SKILL_ROOT
  ? path.join(SKILL_ROOT, 'embedding_map.py')
  : path.join(PROJECT_ROOT, '.claude/skills/career/embedding_map.py');
...
async function runCareer(args: string[]): Promise<unknown> {
  if (gatewayConfigured()) return runSkill('career', args);
  const { stdout } = await execFileAsync(
    'uv',
    ['run', 'python', CAREER_SCRIPT, ...args],
...
async function runForager(args: string[]): Promise<unknown> {
  if (gatewayConfigured()) return runSkill('career', args, { entrypoint: 'career_forager' });
  const { stdout } = await execFileAsync(
    'uv',
...
export async function getLearningPlan() {
  return runCareer(['learning-plan']);
}
...
export async function getPosition(id: string) {
  return runCareer(['show-position', '--id', id]);
}
...
export async function getOpportunity(id: string) {
  return runCareer(['show-opportunity', '--id', id]);
}
...
export async function listPeople() {
  return runCareer(['list-people']);
}

export async function getPerson(id: string) {
  return runCareer(['show-person', '--id', id]);
}
...
export async function listProjects(filters?: {
  ...
  const args = ['list-projects'];
  if (filters?.status) args.push('--status', filters.status);
  if (filters?.role) args.push('--role', filters.role);
  return runCareer(args);
}

export async function updateProject(
  id: string,
  updates: { role?: string; status?: string; url?: string; priority?: string }
) {
...
export async function linkCollaborator(
  ...
  targetId: string,
  role: string,
  options?: { since?: string; strength?: string }
) {
  const args = [
    'link-collaborator',
    '--person', personId,
...
export async function getEmbeddingMap(excludeIds?: string[]) {
  ...
  if (gatewayConfigured()) return runSkill('career', args, { entrypoint: 'embedding_map' });
