...
Usage:
    python .claude/skills/career/career_forager.py <command> [options]
...
Commands:
    ...
    promote             Promote a candidate to a career-position

Examples:
    # Company board sources (require --token)
    python .claude/skills/career/career_forager.py add-source \
    ...
    # Search one source
    python .claude/skills/career/career_forager.py search-source --source "ML Jobs"

    # Full heartbeat
    python .claude/skills/career/career_forager.py heartbeat --min-relevance 0.1

    # List and triage candidates
    python .claude/skills/career/career_forager.py list-candidates --status new
    python .claude/skills/career/career_forager.py triage --id candidate-abc123 --action dismiss

    # Promote to full position
    python .claude/skills/career/career_forager.py promote --id candidate-abc123
...
# =============================================================================
# UTILITIES (shared patterns from career.py)
# =============================================================================
...
def load_position_titles() -> list[str]:
    ...
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = """match $p isa career-position;
                fetch { "name": $p.name };"""
...
def load_user_skills() -> list[dict]:
    """Load career-your-skill entities from TypeDB."""
    skills = []
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = """match $s isa career-your-skill;
                fetch { "slog-skill-name": $s.slog-skill-name, "career-skill-level": $s.career-skill-level };"""
            results = list(tx.query(query).resolve())
            for r in results:
                skills.append({
                    "name": r.get("slog-skill-name", ""),
                    "level": r.get("career-skill-level", "none"),
                })
    return skills
...
def load_existing_position_urls() -> set[str]:
    """Load all career-job-url values from existing career-position entities."""
    urls = set()
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = """match $p isa career-position, has career-job-url $url;
                fetch { "career-job-url": $p.career-job-url };"""
            results = list(tx.query(query).resolve())
            for r in results:
...
def load_existing_candidate_ext_ids() -> set[str]:
    """Load all career-external-job-id values from existing candidates."""
    ext_ids = set()
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = """match $c isa career-candidate, has career-external-job-id $eid;
                fetch { "career-external-job-id": $c.career-external-job-id };"""
            results = list(tx.query(query).resolve())
            for r in results:
                eid = r.get("career-external-job-id")
                if eid:
                    ext_ids.add(eid)
    return ext_ids
...
def load_search_sources() -> list[dict]:
    """Load all career-search-source entities."""
    sources = []
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = """match $s isa career-search-source;
                fetch {
                    "id": $s.id,
                    "name": $s.name,
                    "career-board-token": $s.career-board-token,
                    "career-board-platform": $s.career-board-platform,
                    "alh-company-url": $s.alh-company-url,
                    "career-search-query": $s.career-search-query,
                    "career-search-location": $s.career-search-location
                };"""
            results = list(tx.query(query).resolve())
            for r in results:
                sources.append({
                    "id": r.get("id"),
                    "name": r.get("name"),
                    "board_token": r.get("career-board-token"),
                    "platform": r.get("career-board-platform"),
                    "company_url": r.get("alh-company-url"),
                    "search_query": r.get("career-search-query"),
                    "search_location": r.get("career-search-location"),
                })
    return sources
...
def store_candidates(candidates: list[dict], source_id: str,
    ...
    with get_driver() as driver:
        for c in candidates:
            ...
            insert_query = f'''insert $c isa career-candidate,
                has id "{candidate_id}",
                has name "{escape_string(title)}",
                has career-job-url "{escape_string(c['url'])}",
                has career-external-job-id "{escape_string(c['external_id'])}",
                has career-candidate-status "new",
                has career-relevance-score {c.get('relevance', 0.0)},
                has career-discovered-at {timestamp},
            ...
            try:
                ...
                with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
                    rel_query = f'''match
                        $s isa career-search-source, has id "{source_id}";
                        $c isa career-candidate, has id "{candidate_id}";
                    insert (source: $s, candidate: $c) isa career-source-provides;'''
...
<p style="color:#888;font-size:12px;margin-top:20px">
    ...
    <code>career_forager.py promote --id &lt;candidate-id&gt;</code>
</p>
</body>
</html>"""
...
def cmd_add_source(args):
    ...
    insert_query = f'''insert $s isa career-search-source,
        has id "{source_id}",
        has name "{escape_string(args.name)}",
        has career-board-platform "{args.platform}",
        has created-at {timestamp}'''

    if token:
        insert_query += f', has career-board-token "{escape_string(token)}"'
    if query_kw:
        insert_query += f', has career-search-query "{escape_string(query_kw)}"'
    if location_kw:
        insert_query += f', has career-search-location "{escape_string(location_kw)}"'
    if args.url:
...
def cmd_remove_source(args):
    ...
    # Find the source
    if args.id:
        match_clause = f'$s isa career-search-source, has id "{args.id}"'
    elif args.token:
        match_clause = f'$s isa career-search-source, has career-board-token "{escape_string(args.token)}"'
    elif args.name:
        match_clause = f'$s isa career-search-source, has name "{escape_string(args.name)}"'
    else:
    ...
    with get_driver() as driver:
        ...
        # Delete any career-source-provides relations first
        with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
            try:
                del_rel = f'''match
                    $s isa career-search-source, has id "{source_id}";
                    $r (source: $s, candidate: $c) isa career-source-provides;
                delete $r;'''
...
def cmd_suggest_sources(args):
    ...
    with get_driver() as driver:
        ...
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = """match
                $p isa career-position, has career-job-url $url;
                fetch { "name": $p.name, "career-job-url": $p.career-job-url };"""
            results = list(tx.query(query).resolve())

        # Get existing search sources to exclude
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            src_query = """match $s isa career-search-source;
                fetch { "career-board-token": $s.career-board-token, "career-board-platform": $s.career-board-platform };"""
            existing_sources = list(tx.query(src_query).resolve())

        # Get user skills
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            skill_query = """match $s isa career-your-skill;
                fetch { "slog-skill-name": $s.slog-skill-name, "career-skill-level": $s.career-skill-level };"""
            skill_results = list(tx.query(skill_query).resolve())

    existing_tokens = set()
    for s in existing_sources:
        token = s.get("career-board-token")
        if token:
...
def cmd_list_candidates(args):
    ...
    with get_driver() as driver:
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = "match $c isa career-candidate"

            if args.status:
                query += f', has career-candidate-status "{args.status}"'

            query += """;
                fetch {
                    "id": $c.id,
                    "name": $c.name,
                    "career-job-url": $c.career-job-url,
                    "location": $c.alh-location,
                    "career-relevance-score": $c.career-relevance-score,
                    "career-candidate-status": $c.career-candidate-status,
                    "career-external-job-id": $c.career-external-job-id,
                    "career-discovered-at": $c.career-discovered-at,
                    "career-triage-reason": $c.career-triage-reason
                };"""
            ...
            if args.source:
                src_query = f'''match
                    $s isa career-search-source, has career-board-token "{escape_string(args.source)}";
                    (source: $s, candidate: $c) isa career-source-provides;
                fetch {{ "id": $c.id }};'''
    ...
    for r in results:
        candidates.append({
            ...
            "relevance": r.get("career-relevance-score"),
            "status": r.get("career-candidate-status"),
            "external_id": r.get("career-external-job-id"),
            "discovered_at": r.get("career-discovered-at"),
            "triage_reason": r.get("career-triage-reason"),
        })
...
def cmd_triage(args):
    ...
    with get_driver() as driver:
        # Check existence
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            check = f'''match $c isa career-candidate, has id "{args.id}";
                fetch {{ "id": $c.id, "name": $c.name, "career-candidate-status": $c.career-candidate-status }};'''
            existing = list(tx.query(check).resolve())
        ...
        with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
            update_query = f'''match
                $c isa career-candidate, has id "{args.id}", has career-candidate-status $old;
            delete $c has $old;'''
        ...
        with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
            insert_query = f'''match
                $c isa career-candidate, has id "{args.id}";
            insert $c has career-candidate-status "{args.action}";'''
            tx.query(insert_query).resolve()
            tx.commit()
...
def cmd_promote(args):
    ...
    with get_driver() as driver:
        # Load candidate
        with driver.transaction(TYPEDB_DATABASE, TransactionType.READ) as tx:
            query = f'''match $c isa career-candidate, has id "{args.id}";
                fetch {{
                    "id": $c.id,
                    "name": $c.name,
                    "career-job-url": $c.career-job-url,
        ...
        url = candidate.get("career-job-url") or ""
        ...
        pos_query = f'''insert $p isa career-position,
            has id "{position_id}",
            has name "{escape_string(title)}",
            has created-at {timestamp}'''

        if url:
            pos_query += f', has career-job-url "{escape_string(url)}"'
        if location:
        ...
        with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
            tx.query(f'''match
                $c isa career-candidate, has id "{args.id}";
            insert $c has career-candidate-status "promoted";''').resolve()
            tx.commit()
...
def cmd_add_candidate(args):
    ...
    query = f'''insert $c isa career-candidate,
        has id "{candidate_id}",
        has name "{escape_string(args.title)}",
        has career-candidate-status "reviewed",
        has career-discovered-at {timestamp}'''

    if args.url:
        query += f', has career-job-url "{escape_string(args.url)}"'
    if args.location:
        query += f', has alh-location "{escape_string(args.location)}"'
    if args.relevance is not None:
        query += f', has career-relevance-score {args.relevance}'
    if args.reason:
        query += f', has career-triage-reason "{escape_string(args.reason)}"'
    ...
    with get_driver() as driver:
        ...
        if args.source_id:
            try:
                with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
                    tx.query(f'''match
                        $s isa career-search-source, has id "{escape_string(args.source_id)}";
                        $c isa career-candidate, has id "{candidate_id}";
                    insert (source: $s, candidate: $c) isa career-source-provides;''').resolve()
                    tx.commit()
            except Exception:
        ...
        try:
            with driver.transaction(TYPEDB_DATABASE, TransactionType.WRITE) as tx:
                tx.query(f'''match
                    ...
                    $c isa career-candidate, has id "{candidate_id}";
                insert (seeker: $role, opportunity: $c) isa career-seeker-pipeline;''').resolve()
                tx.commit()
        except Exception:
...
def main():
    ...
    p.add_argument("--source", help="Filter by source career-board-token")
